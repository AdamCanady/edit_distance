      ...
      if (config.mode ==
        ...
        for (int i = 0; i < result.numLocations; i++) {
          ...
          myersCalcEditDistanceSemiGlobal(
              ...
              result.editDistance, EDLIB_MODE_SHW, &bestScoreSHW, &positionsSHW,
              &numPositionsSHW);
          // Taking last location as start ensures that alignment will not start
          // with insertions if it can start with mismatches instead.
  ...
  for (int c = 0; c < targetLength; c++) {  // for each column
    ...
    if (mode != EDLIB_MODE_HW) {
      ...
      if (c % STRONG_REDUCE_NUM ==
          0) {  // Do strong reduction every some blocks
        while (firstBlock <= lastBlock &&
 ...
 * otherwise it is NULL. Make sure to free this array
 * using delete[].
...
static int obtainAlignmentTraceback(const int queryLength,
  ...
  *alignmentLength = 0;
  ...
  currM <<= W;
  int blockPos =
      WORD_SIZE - W - 1;  // 0 based index of current cell in blockPos
  ...
  while (true) {
    ...
    // Move left - deletion from target - insertion to query
      ...
      if (c > 0 && b >= alignData->firstBlocks[c - 1] &&
        ...
        if (c ==
            0) {  // If there are no cells to the left (only boundary cells)
          thereIsLeftBlock = true;
          lScore = b * WORD_SIZE + blockPos + 1;
          ulScore = lScore - 1;
        } else {
...
extern "C" EdlibAlignConfig edlibNewAlignConfig(
    int k, EdlibAlignMode mode, EdlibAlignTask task,
    EdlibEqualityPair* additionalEqualities, int additionalEqualitiesLength) {
